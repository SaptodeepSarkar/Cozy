"""Strip zero-pad tails from cached mels, retrain CozyNet on cleaned mels,
export ONNX + meta. Fast (~3 min on GPU, 0 VRAM OOM risk)."""
import json
import time
...
Xtr = torch.from_numpy(X_norm[train_idx]).float()
ytr = torch.from_numpy(y[train_idx]).float()
Xva = torch.from_numpy(X_norm[test_idx]).to(device)
yva = torch.from_numpy(y[test_idx]).float().numpy()
